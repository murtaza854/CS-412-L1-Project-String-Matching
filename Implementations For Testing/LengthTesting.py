...
def plotter(r_k_time, b_f_time, kmp_time, time, filename):
    ...
    '''
    plt.title(f'Complexity Analysis when m = 500')
...
start_test,end_test,step_test = 5000,1000001,5000
...
for i,n in enumerate(range(start_test,end_test,step_test)):
    pattern = get_random_string(500,ascii_lowercase)
    string_to_search = get_random_string(n-len(pattern), ascii_lowercase)
    ...
    if n % 100 == 0:
        print(n)
print("done")
plotter(r_k_temp, b_f_temp, kmp_temp, times, "n_analysis")